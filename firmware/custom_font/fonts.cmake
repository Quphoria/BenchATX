...
set(GEN_SCRIPT ${CMAKE_CURRENT_LIST_DIR}/font_to_header.py)
...
add_custom_command(
    ...
    DEPENDS ${Pix32_Font_PATH} ${GEN_SCRIPT} font_venv/ready FONT_VENV # target level dependency to avoid race condition
    COMMAND ./font_venv/${VENV_BIN_DIR}/python "${GEN_SCRIPT}"
            -W 6 -H 12
            "${Pix32_Font_PATH}"
            -o "${Pix32_Font_H_PATH}"
            Pix32_Font
...
add_custom_command(
    ...
    DEPENDS ${On_Icon_PATH} ${GEN_SCRIPT} font_venv/ready FONT_VENV # target level dependency to avoid race condition
    COMMAND ./font_venv/${VENV_BIN_DIR}/python "${GEN_SCRIPT}"
            -W 13 -H 11 -s 65 -c 1
            "${On_Icon_PATH}"
            -o "${On_Icon_H_PATH}"
...
add_custom_command(
    ...
    DEPENDS ${Chan_Icons_PATH} ${GEN_SCRIPT} font_venv/ready FONT_VENV # target level dependency to avoid race condition
    COMMAND ./font_venv/${VENV_BIN_DIR}/python "${GEN_SCRIPT}"
            -W 32 -H 16 -s 65 -c 5
            "${Chan_Icons_PATH}"
            -o "${Chan_Icons_H_PATH}"
